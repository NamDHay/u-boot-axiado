	...
	pci_addr_t *bar, bool supports_64bit)
{
	...
	if (addr - res->bus_start + size > res->size) {
		pr_err("No room in resource, avail start=%llx / size=%llx, "
	...
	if (upper_32_bits(addr) && !supports_64bit) {
		pr_err("Cannot assign 64-bit address to 32-bit-only resource\n");
		goto error;
	}
	...
	*bar = addr;
    ...
    printf("allocate:\n");
    printf(" region start : %016llx\n", res->bus_start);
    printf(" region next  : %016llx\n", res->bus_lower);
    printf(" size         : %016llx\n", res->size);
