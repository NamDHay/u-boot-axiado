...
static int cdns_i3c_master_disable(struct cdns_i3c_master *master)
{
	...
	return readl_poll_timeout(master->regs + MST_STATUS0, status,
				  status & MST_STATUS0_IDLE, 1000000);
}
					...
					struct i3c_ccc_cmd *cmd)
{
    ...
    master->xferqueue.cur = xfer;
    cdns_i3c_master_start_xfer_locked(master);
    cdns_i3c_master_end_xfer_locked(master);
    ...
    printf("Send ccc cmd status %d\n", xfer->ret);
    printf("Send ccc cmd error %d\n", cmd->err);
...
static int cdns_i3c_master_probe(struct udevice *dev)
{
	...
	master->devdata = (struct cdns_i3c_data *)dev_get_driver_data(dev);
	if (!master->devdata)
		return -EINVAL;
	...
	master->sysclk = devm_clk_get(dev, "sysclk");
	if (IS_ERR(master->sysclk))
		return PTR_ERR(master->sysclk);

	ret = clk_enable(master->pclk);
	if (ret)
		return ret;

	ret = clk_enable(master->sysclk);
	if (ret)
		goto err_disable_pclk;

	/* Parse device tree properties for additional quirks */
	if (ofnode_read_bool(dev_ofnode(dev), "enable-repeated-start"))
		master->quirks |= CDNS_I3C_QUIRKS_ENABLE_REPEATED_START;

	if (ofnode_read_bool(dev_ofnode(dev), "disable-ibir"))
		master->quirks |= CDNS_I3C_QUIRKS_DISABLE_IBIR_HANDLING;

	if (ofnode_read_bool(dev_ofnode(dev), "disable-ibir-error"))
		master->quirks |= CDNS_I3C_QUIRKS_DISABLE_IBIR_ERROR;

	if (ofnode_read_bool(dev_ofnode(dev), "disable-ctrl-halt"))
		master->quirks |= CDNS_I3C_QUIRKS_DISABLE_CTRL_HALT;

	if (ofnode_read_bool(dev_ofnode(dev), "disable-mst-abort"))
		master->quirks |= CDNS_I3C_QUIRKS_DISABLE_MST_ABORT;

	if (ofnode_read_bool(dev_ofnode(dev), "skip-payload-inc"))
	...
	master->ibi.slots = calloc(master->ibi.num_slots,
...
static const struct udevice_id cdns_i3c_master_ids[] = {
	{ .compatible = "cdns,i3c-master", .data = (ulong)&cdns_i3c_devdata },
	{ .compatible = "axiado,ax3000-i3c", .data = (ulong)&ax3000_i3c_def },
	{ /* sentinel */ },
};

U_BOOT_DRIVER(cdns_i3c_driver) = {
    .name = "cdns-i3c-master",
	.id = UCLASS_I3C,
	.of_match = cdns_i3c_master_ids,
	.probe = cdns_i3c_master_probe,
	.priv_auto = sizeof(struct cdns_i3c_master),
	.ops    = &cdns_i3c_master_ops,
};
