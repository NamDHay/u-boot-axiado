...
struct ax_emmc_ops {
    ...
    /* Set bus speed */
    int (*set_speed)(emmc_speed_mode_t speed_mode);

    /* Set bus width: 1, 4 or 8 bits */
    int (*set_bus_width)(emmc_bus_width_t bus_width);

    /* Read blocks from eMMC */
    int (*read)(uint32_t start, uint32_t blkcnt, void *dst);

    /* Write blocks to eMMC */
    int (*write)(uint32_t start, uint32_t blkcnt, const void *src);

    /* Erase blocks */
    int (*erase)(uint32_t start, uint32_t blkcnt);
...
static inline int ax_emmc_set_speed(emmc_speed_mode_t speed_mode)
{
    if (emmc.set_speed == NULL)
        return -ENOSYS;

    return emmc.set_speed(speed_mode);
}

static inline int ax_emmc_set_bus_width(emmc_bus_width_t bus_width)
{
    if (emmc.set_bus_width == NULL)
        return -ENOSYS;

    return emmc.set_bus_width(bus_width);
}

static inline int ax_emmc_read(uint32_t start, uint32_t blkcnt,
        void *dst)
{
    if (emmc.read == NULL)
        return -ENOSYS;
...
static inline int ax_emmc_write(uint32_t start, uint32_t blkcnt,
        const void *src)
{
    if (emmc.write == NULL)
        return -ENOSYS;
...
static inline int ax_emmc_erase(uint32_t start, uint32_t blkcnt)
{
    if (emmc.erase == NULL)
        return -ENOSYS;
...
static inline uint32_t ax_emmc_get_block_count(void)
{
